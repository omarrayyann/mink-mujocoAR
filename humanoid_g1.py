...
if __name__ == "__main__":
    ...
    with mujoco.viewer.launch_passive(
        ...
        mujocoAR = MujocoARConnector(mujoco_model=model, mujoco_data=data, port=9999)
        mujocoAR.start()
        initial_pos = data.site("left_palm").xpos.copy()
        initial_mat = data.site("left_palm").xmat.copy().reshape((3,3))
        mujocoAR.link_body(
            name="left_palm_target",
            scale=2.0,
        ...
        mujocoAR = MujocoARConnector(mujoco_model=model, mujoco_data=data, port=1111)
        mujocoAR.start()
        initial_pos = data.site("right_palm").xpos.copy()
        initial_mat = data.site("right_palm").xmat.copy().reshape((3,3))
        mujocoAR.link_body(
            name="right_palm_target",
            scale=2.0,
            position_origin=initial_pos,
            rotation_origin=initial_mat,
            disable_rot=True,
